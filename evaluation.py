...
def get_next_day_prediction(stock_symbol, q_network_path):
    env = StockEnv(test=True, stock=stock_symbol)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    q_network = dqn.QNetworkCNN(action_dim=env.action_space.n).to(device)
...
if __name__ == '__main__':
    ...
    performance = evaluate(q_network_path, "NVDA", total_capital)

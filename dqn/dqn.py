...
def evaluate_one_state(Qmodel, state):
    """
    Evaluates a single state with the Q-Network and returns the greedy action.
    Args:
        Qmodel: The Q-network model.
        state: A single input state (not a batch of states).

    Returns:
        selected_action: The greedy action based on the Q-values.
    """
    Qmodel.eval()
    ...
    action_map = [-1, 0, 1]  # Example mapping
    selected_action = action_map[action]

    Qmodel.train()

    return selected_action, values.cpu().numpy()[0][action]
